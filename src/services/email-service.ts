...
export const allProducstVerifyService = async (user: string, title: string) => {
  const database: ProductsFinalModel [] = await mylistRepository(user, title, "desc");
  ...
  if (database) {
    ...
    await sendEmail2(userInfo.email, "Produtos", "www....", userInfo.name,ProdutosPreçoDesejado, ProdutosMenorPrecoHistórico  )
